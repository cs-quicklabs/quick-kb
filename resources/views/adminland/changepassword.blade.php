    ...
    <div>
        ...
        <form class="w-full mt-6" action="{{ route('adminland.updatepassword') }}" method="POST">
            ...
            <div class="mb-5 mt-6">
                ...
                <input type="text" name="old_password_or_recovery_code" id="old_password_or_recovery_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-{{$color}}-500 focus:border-{{$color}}-500 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-{{$color}}-500 dark:focus:border-{{$color}}-500" placeholder="••••••••" required>
            ...
            <div class="mb-5">
                ...
                <input type="password" name="new_password" id="new_password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-{{$color}}-500 focus:border-{{$color}}-500 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-{{$color}}-500 dark:focus:border-{{$color}}-500" placeholder="••••••••" required>
                @error('new_password')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div> 
            ...
            <button type="submit" class="text-white bg-{{$color}}-700 hover:bg-{{$color}}-800 focus:ring-4 focus:outline-none focus:ring-{{$color}}-300 font-medium rounded text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-{{$color}}-600 dark:hover:bg-{{$color}}-700 dark:focus:ring-{{$color}}-800">Save</button>
        </form>
    </div>
</main>
